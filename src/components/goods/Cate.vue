  ...
  <div>
    ...
    <el-dialog title="提示" :visible.sync="editCategoriesDialogVisible" width="50%">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="editGoodsCategories">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async getParentCateList() {
      ...
      if (res.meta.status !== 200) {
        this.$message.error('获取父级分类的数据列表失败')
      }
    ...
    parentCateChange() {
      ...
      //如果selectedKeys 数组中的length大于0证明选中
      //的父级分类
    ...
    //展示商品分类对话框
    async showEditGoodsCategoriesDialog(id) {
